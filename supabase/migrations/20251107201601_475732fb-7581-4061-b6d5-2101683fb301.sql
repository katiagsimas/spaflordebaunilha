-- Função corrigida para deletar dados seletivamente por módulo (sem conflito com triggers)
...
BEGIN
  ...
  IF (p_selecao->>'planoContas')::BOOLEAN THEN
    -- Remover flag padrao_sistema para evitar trigger de proteção
    UPDATE plano_contas 
    SET padrao_sistema = false 
    WHERE (user_id = p_user_id OR padrao_sistema = true)
  ...
  IF (p_selecao->>'categoriasPlanoContas')::BOOLEAN THEN
    -- Primeiro deletar plano_contas associado a essas categorias
    UPDATE plano_contas 
    SET padrao_sistema = false 
