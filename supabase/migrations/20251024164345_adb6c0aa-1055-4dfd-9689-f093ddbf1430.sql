-- Adicionar deleção de tipos_insumos do usuário APÓS deletar ingredientes/embalagens
...
BEGIN
  ...
  -- Deletar ingredientes (antes de tipos_insumos)
  DELETE FROM ingredientes WHERE usuario_id = p_user_id;
  
  -- Deletar embalagens (antes de tipos_insumos)
  DELETE FROM embalagens WHERE usuario_id = p_user_id;
  
  -- AGORA deletar tipos_insumos do usuário (após ingredientes/embalagens)
  ...
  -- NOTA: Mantém unidades_medida, tags_encomendas, plano_contas, 
  -- categorias_plano_contas, bancos, tipos_documento pois são 
