-- Remover triggers de proteção que impedem exclusão administrativa
-- A segurança dos dados já é garantida pelas políticas RLS

DROP TRIGGER IF EXISTS proteger_contas_padrao ON plano_contas;
DROP TRIGGER IF EXISTS impedir_exclusao_padrao ON plano_contas;
DROP TRIGGER IF EXISTS proteger_categorias_padrao ON categorias_plano_contas;

-- Função simplificada para deletar dados seletivamente
