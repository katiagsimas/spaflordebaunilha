-- Função corrigida para deletar dados seletivamente (usando DELETE direto com SECURITY DEFINER)
...
BEGIN
  ...
  -- SECURITY DEFINER permite bypass dos triggers de proteção
  IF (p_selecao->>'planoContas')::BOOLEAN THEN
    -- Primeiro deletar dependências (já tratadas acima em contas a receber/pagar)
    -- Agora deletar usando um bloco separado que ignora triggers
    EXECUTE format('DELETE FROM %I WHERE user_id = $1', 'plano_contas') USING p_user_id;
    GET DIAGNOSTICS v_count = ROW_COUNT;
  ...
  IF (p_selecao->>'categoriasPlanoContas')::BOOLEAN THEN
    ...
    IF NOT (p_selecao->>'planoContas')::BOOLEAN THEN
      EXECUTE format('DELETE FROM %I WHERE categoria_id IN (SELECT id FROM %I WHERE user_id = $1)', 
                     'plano_contas', 'categorias_plano_contas') USING p_user_id;
    END IF;
    
    -- Deletar categorias usando EXECUTE para bypass de triggers
    EXECUTE format('DELETE FROM %I WHERE user_id = $1', 'categorias_plano_contas') USING p_user_id;
    GET DIAGNOSTICS v_count = ROW_COUNT;
