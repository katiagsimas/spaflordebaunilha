-- Função corrigida para deletar dados seletivamente por módulo
...
BEGIN
  ...
  IF (p_selecao->>'planoContas')::BOOLEAN THEN
    DELETE FROM plano_contas WHERE user_id = p_user_id OR padrao_sistema = true;
    GET DIAGNOSTICS v_count = ROW_COUNT;
  ...
  IF (p_selecao->>'categoriasPlanoContas')::BOOLEAN THEN
    DELETE FROM categorias_plano_contas WHERE user_id = p_user_id OR padrao_sistema = true;
    GET DIAGNOSTICS v_count = ROW_COUNT;
