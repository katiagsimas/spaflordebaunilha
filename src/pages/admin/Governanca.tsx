        ...
        <div className="grid gap-4 sm:grid-cols-2 max-w-4xl">
          {cards.map(({ title, description, icon: Icon, url }) => (
            <Link key={url} to={url} className="group">
              <Card className="h-full transition-all duration-200 hover:shadow-lg border-sfb-areia/40 hover:border-sfb-terracota cursor-pointer">
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="h-11 w-11 rounded-lg bg-sfb-terracota/10 flex items-center justify-center group-hover:bg-sfb-terracota/15 transition-colors">
                        <Icon className="h-5 w-5 text-sfb-terracota" />
                      </div>
                      <CardTitle className="font-display text-lg text-sfb-cacau">{title}</CardTitle>
                    </div>
                    <ChevronRight className="h-5 w-5 text-muted-foreground group-hover:text-sfb-terracota group-hover:translate-x-0.5 transition-all" />
                  </div>
                </CardHeader>
                <CardContent>
                  <CardDescription className="font-body text-sm text-sfb-cacau/70">
                    {description}
                  </CardDescription>
                </CardContent>
              </Card>
