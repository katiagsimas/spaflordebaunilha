...
export function usePlanejamento() {
  ...
  const calcularFaturamentoMesAnterior = (): number => {
    ...
    const mesAnterior = mesAtual === 0 ? 11 : mesAtual - 1;
    const anoAnterior = mesAtual === 0 ? anoAtual - 1 : anoAtual;
    return calcularFaturamentoMes(mesAnterior, anoAnterior);
  };
  ...
  const calcularProjecaoVendas = (): { projecao: number; tendencia: 'crescimento' | 'estavel' | 'queda' } => {
    ...
    // Calcular últimos 3 meses baseado nas encomendas
    const ultimosTresMeses = [-2, -1, 0].map(offset => {
      const mes = mesAtual + offset;
      const ano = mes < 0 ? anoAtual - 1 : anoAtual;
      const mesAjustado = mes < 0 ? 12 + mes : mes;
      return calcularFaturamentoMes(mesAjustado, ano);
    });
